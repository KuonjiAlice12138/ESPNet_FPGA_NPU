# Round 2 CSim only. Keep the GUI production top/config unchanged.
...
if {[info exists ::env(ESP_INT8_ROUND2_TEST)]} {
  if {$::env(ESP_INT8_ROUND2_TEST) eq "dual"} {
    set tests {binary2 cityscapes20}
  } else {
    set tests [list $::env(ESP_INT8_ROUND2_TEST)]
  }
}
foreach test $tests {
  if {$test ni {focus binary2 cityscapes20}} {error "Unknown test: $test"}
  set profile [expr {$test eq "focus" ? "binary2" : $test}]
  set ::env(ESP_INT8_CSIM_ARTIFACT_DIR) [file join $repo hw_artifacts ${profile}_int8_h256w512_r2_v4]
  open_project -reset [file join $root hls_work_0910_round2_$test]
  if {$test eq "focus"} {
    add_files [file join $root src win_gen.cpp]
    set_top scheduled_window_generator_row
    add_files -tb [file join $root tb c3_row_reuse_tb.cpp]
  } else {
    ...
    add_files -tb -cflags "-DESP_INT8_CSIM_CLASS_COUNT=$classes -DESP_INT8_CSIM_VALIDATE_ROW_REUSE" [file join $root tb top_golden_sample_tb.cpp]
  }
