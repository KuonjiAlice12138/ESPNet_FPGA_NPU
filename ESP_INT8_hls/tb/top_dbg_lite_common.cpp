...
int main() {
    ...
    constexpr std::size_t kDumpWords = static_cast<std::size_t>(DBG_DUMP_WORDS);
    static_assert(kDumpWords > 0U, "DBG_DUMP_WORDS must be positive.");
    ...
    const std::uint32_t dump_words = static_cast<std::uint32_t>(kDumpWords);
    ...
    std::printf("debug lite %s: stop_after=%u dump_tensor=%u dump_words=%u\n",
                DBG_NAME,
                stop_after,
                dump_tensor,
                dump_words);
    ...
    std::printf("debug lite %s wrote %s bytes=%zu nonzero=%d checksum=0x%08x\n",
                DBG_NAME,
                out_path,
                output.size(),
                nonzero,
                checksum);
    std::printf("debug lite %s dbg status=0x%08x hb=0x%08x act=%u wgt=%u psum=%u out=%u\n",
                DBG_NAME,
    ...
    std::printf("debug lite %s first bytes:", DBG_NAME);
